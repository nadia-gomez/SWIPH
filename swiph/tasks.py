...
from .models import BasicMachineLearningDB, MachineLearningDB, ScriptTracker, ErrorCode

@app.task(name='swiph.retrain')
...
def basic_start_dbcreation(self, annualHourly):
    """
    ...
    while True:
        ...
        if not ScriptTracker.objects.filter(name="create_basic_mldb").latest('starting_date').keep_running:
            break

        #Prepare random values
        
        location = locations[np.random.randint(len(locations))]
        rv = np.random.rand(2) #rv=random_values Just to create all the random number at once.

        inputs={
            'T_cold_ref':round(np.random.uniform(low=inputs_sam['T_fp'],high=150),2), #[0,150)
            'P_turb_des':round(rv[1]*20,2), # Solarfield Heat sink inlet pressure [bar] #[0,20)
        ...
        inputs_sam.update({'meteoFile' : os.path.dirname(os.path.dirname(__file__))+"/solar_resource/"+location[0],})
...
@shared_task(bind=True)
def start_dbcreation(self, annualHourly):
    """
    Param: annualHourly list. It doesn't need to have anything special
    Return: None. It only feeds the database.
